...
$namep = $row1["name"];
$surnamep = $row1["lastname"];
$pp = $row1["pp"];

//following list
$followinglist = $db->prepare("SELECT following_id FROM following WHERE user_id = :id");
...
//followers list
$followerslist = $db->prepare("SELECT user_id FROM following WHERE following_id= :id");
...
//favorite films
$favmovies = $db->prepare("SELECT movie_id FROM favoritemovies WHERE user_id= :id");
...
//watchlistu
$watchlistu = $db->prepare("SELECT movie_id FROM watchlist WHERE user_id= :id");
...
//watchedu
$watchedu = $db->prepare("SELECT movie_id FROM watchedmovies WHERE user_id= :id");
...
<div class="row" style="margin-top: 70px;  margin-left: 0px;">
    <div class="col">
        <img class="ppbig" src="userspp/<?php echo $pp ?>">
    </div>
    <div class="col-sm-" style="width: 200px">
        <div class="row" style="text-align: center">
            <p class="nameheader"><?php echo $namep, "  ", $surnamep ?></p>

        </div>
        <div class="row" style="text-align: center">
            <p>
                <button class=" btn btn-sm  editprofile" onclick="location.href='editprofile.php'">EDIT PROFILE</button>
            </p>
        </div>
    </div>
                    ...
                    <div style="height:350px">
                        ...
                        foreach ($rows as $row) {
                            ...
                            <form>
                                ...
                                <button class=" btn btn-sm  followingbutton vertical-center"
                                        value="<?php echo $fuserid ?>" name="followingbutton" type="submit">Unfollow
                                </button>
                            </form>
                            <?php
                        }
                    ...
                    <div style="height:350px">
                        ...
                        foreach ($rows2 as $row) {
                            ...
                            <form>
                                ...
                                <button class=" btn btn-sm  followingbutton vertical-center"
                                        value="<?php echo $fuserid2 ?>" name="followers_remove" type="submit">Remove
                                </button>
                            </form>
                            <?php
                        }
...
<div class="row" style=" margin-top: 25px; margin-left: 20px">
    ...
    foreach ($watchedrow as $row) {
        ...
        <div class="score">
            <div class=" wscore"><?php echo $rrow['rate']?>/10</div>
...
if (isset($_REQUEST['followingbutton'])) {
    $unfollow = $db->prepare("DELETE FROM following WHERE user_id = :id and following_id = :fuserid");
    if ($unfollow->execute(array(
        ':id' => $id,
        ':fuserid' => $fuserid,
    )))
        header("refresh:5; profilepage.php");
...
if (isset($_REQUEST['followers_remove'])) {

    $remove = $db->prepare("DELETE FROM following WHERE user_id = :fuserid and following_id = :id");
    if ($remove->execute(array(
        ':fuserid' => $fuserid2,
        ':id' => $id,
    )))
        header("refresh:6; profilepage.php");
}
*/?>
